...
/**
 * Servlet implementation class FileUploadSerblet6
 */
...
//		logger.debug("part getName : "+uploadFilePart.getName());
		...
		//file에 이름에 대한 정보를 획득 : getHeaer("content-Disposition");
		String contentDisposition = uploadFilePart.getHeader("Content-Disposition");
		logger.debug("contentDispssition : " + contentDisposition);
		...
		//파일 저장 경로 설정
		//실무 : 별도 storage
		//개발환경: 임의 영역(서버 변경시, 제기동시 업로드 파일 삭제됨)
		...
		//2.getServletContext() (서블릿서 제공해주는 메서드)
