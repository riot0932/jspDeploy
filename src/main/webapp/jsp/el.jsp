...
<body>
	...
	<h3>request</h3>
	표현식 :
	<%=request.getAttribute("requestScop")%><br> 
	EL :${requestScope.requestScop}<br>
	${requestScop}

	<h3>session</h3>
	표현식 :
	<%=session.getAttribute("sessionScop")%><br> 
	EL :${sessionScope.sessionScop}<br> 
	${sessionScop}

	<h3>application</h3>
	표현식 :
	<%=application.getAttribute("applicationScop")%><br> 
	...
	${sameName} 어떤 영역의 값을 가져오나?? --> 가장 작은 영역에서 부터 큰영역으로 검색 pageContext ->
